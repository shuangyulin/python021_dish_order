...
def caipinxinxi_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        msg['data']['pageSize']  =caipinxinxi.page(caipinxinxi, caipinxinxi, req_dict, request)
...
def caipinxinxi_thumbsup(request,id_):
    '''
    ...
    if request.method in ["POST", "GET"]:
        ...
        rets=caipinxinxi.getbyid(caipinxinxi,caipinxinxi,id_)
        ...
        error = caipinxinxi.updatebyparams(caipinxinxi,caipinxinxi, update_dict)
        if error!=None:
            msg['code'] = crud_error_code
            msg['msg'] = error
        return JsonResponse(msg)
...
def caipinxinxi_info(request,id_):
    '''
    '''
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}

        data = caipinxinxi.getbyid(caipinxinxi,caipinxinxi, int(id_))
        if len(data)>0:
            msg['data']=data[0]
        #浏览点击次数
        try:
            __browseClick__= caipinxinxi.__browseClick__
        except:
            __browseClick__=None

        if __browseClick__=="是"  and  "clicknum"  in caipinxinxi.getallcolumn(caipinxinxi,caipinxinxi):
            try:
                clicknum=int(data[0].get("clicknum",0))+1
            except:
                clicknum=0+1
            click_dict={"id":int(id_),"clicknum":clicknum}
            ret=caipinxinxi.updatebyparams(caipinxinxi,caipinxinxi,click_dict)
            if ret!=None:
                msg['code'] = crud_error_code
                msg['msg'] = ret
        return JsonResponse(msg)

def caipinxinxi_detail(request,id_):
    '''
    '''
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
...
def caipinxinxi_update(request):
    '''
    '''
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        ...
        if req_dict.get("mima") and req_dict.get("password"):
            if "mima" not  in caipinxinxi.getallcolumn(caipinxinxi,caipinxinxi) :
                del req_dict["mima"]
            if  "password" not  in caipinxinxi.getallcolumn(caipinxinxi,caipinxinxi) :
...
def caipinxinxi_delete(request):
    '''
    批量删除
    '''
    if request.method in ["POST", "GET"]:
        ...
        error=caipinxinxi.deletes(caipinxinxi,
            caipinxinxi,
             req_dict.get("ids")
        )
        if error!=None:
...
def caipinxinxi_vote(request,id_):
    '''
    ...
    if request.method in ["POST", "GET"]:
        ...
        for i in data:
            ...
            if votenum!=None:
                ...
                error=caipinxinxi.updatebyparams(caipinxinxi,caipinxinxi,params)
                if error!=None:
                    msg['code'] = crud_error_code
                    msg['msg'] = error
        return JsonResponse(msg)

def caipinxinxi_importExcel(request):
    if request.method in ["POST", "GET"]:
        ...
        if file_type in ['xlsx', 'xls']:
            ...
            try:
                for row in range(1, rows):
                    ...
                    req_dict = {}
                    caipinxinxi.createbyreq(caipinxinxi, caipinxinxi, req_dict)
...
def caipinxinxi_sendemail(request):
    if request.method in ["POST", "GET"]:
        req_dict = request.session.get("req_dict")
...
def caipinxinxi_autoSort2(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        leixing = set()
        try:
            cursor.execute("select goodtype from orders where userid = %d"%(request.session.get("params").get("id"))+" and tablename = 'caipinxinxi' order by addtime desc")
        ...
        L = []
        cursor.execute("select * from caipinxinxi where caipinleixing in ('%s"%("','").join(leixing)+"') union all select * from caipinxinxi where caipinleixing not in('%s"%("','").join(leixing)+"')")
...
def caipinxinxi_value(request, xColumnName, yColumnName, timeStatType):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": {}}
        ...
        sql = ''
        if timeStatType == '日':
            sql = "SELECT DATE_FORMAT({0}, '%Y-%m-%d') {0}, sum({1}) total FROM caipinxinxi {2} GROUP BY DATE_FORMAT({0}, '%Y-%m-%d')".format(xColumnName, yColumnName, where, '%Y-%m-%d')

        if timeStatType == '月':
            sql = "SELECT DATE_FORMAT({0}, '%Y-%m') {0}, sum({1}) total FROM caipinxinxi {2} GROUP BY DATE_FORMAT({0}, '%Y-%m')".format(xColumnName, yColumnName, where, '%Y-%m')

        if timeStatType == '年':
            sql = "SELECT DATE_FORMAT({0}, '%Y') {0}, sum({1}) total FROM caipinxinxi {2} GROUP BY DATE_FORMAT({0}, '%Y')".format(xColumnName, yColumnName, where, '%Y')
        L = []
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
...
def caipinxinxi_o_value(request, xColumnName, yColumnName):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": {}}
        
        where = ' where 1 = 1 '
        
        sql = "SELECT {0}, sum({1}) AS total FROM caipinxinxi {2} GROUP BY {0}".format(xColumnName, yColumnName, where)
        L = []
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
